-- Простое исправление для порядковых номеров заказов
...
-- 3. Обновляем существующие заказы порядковыми номерами (простой способ)
DO $$
DECLARE
    order_record RECORD;
    counter INTEGER := 1;
BEGIN
    -- Проходим по всем заказам в порядке создания и присваиваем номера
    FOR order_record IN 
